...
def _update_users_content_roles(user, idurl, content_roles):
	...
	"""
	member = component.getAdapter(user, IMutableGroupMember, nauth.CONTENT_ROLE_PREFIX)
	...
	for package in (library.contentPackages if library is not None else ()):
		if get_provider(package.ntiid).lower() == provider:
			provider_packages[package.title.lower()].append(get_specific(package.ntiid))
...
update_users_content_roles = _update_users_content_roles
