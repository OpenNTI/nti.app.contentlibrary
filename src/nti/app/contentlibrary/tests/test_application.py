...
class _ApplicationLibraryTestLayer(ApplicationTestLayer):
    ...
    def testSetUp(cls, test=None):
        ...
        cls.cur_lib = registry.queryUtility(IContentPackageLibrary)
    ...
    def testTearDown(cls):
        test = find_test()
        registry = test.config.registry
        registry.unregisterUtility(cls.new_lib, IContentPackageLibrary)
        if cls.cur_lib is not None and test.config.registry is component.getGlobalSiteManager():
            registry.registerUtility(cls.cur_lib, IContentPackageLibrary)
...
class TestApplicationLibraryBase(ApplicationLayerTest):
    ...
    child_ntiid = ntiids.make_ntiid(provider=u'ou',
                                    specific=u'test2',
                                    nttype=u'HTML')
    child_ordinal = 0
    ...
    def _setup_library(self, content_root='/prealgebra/', lastModified=None):
        ...
        class NIDMapper(object):
            ...
            def __init__(self, context):
                root_package = traversal.find_interface(context, IContentPackage)
...
class TestApplicationLibraryNoSlash(TestApplicationLibrary):
    ...
    def _setup_library(self, *unused_args, **kwargs):
