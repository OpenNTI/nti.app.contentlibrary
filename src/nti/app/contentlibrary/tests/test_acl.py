...
class TestLibraryEntryAclProvider(ApplicationLayerTest):
    ...
    def setUpClass(cls):
        ...
        class Key(object):
            ...
            def __init__(self, unused_bucket=None, name=None):
        ...
        path = os.path.join(cls.temp_dir, 'index.html')
        cls.library_entry.key = Key(name=path)
    ...
    def test_no_acl_file(self):
        acl_prov = IACLProvider(self.library_entry)
        assert_that(acl_prov, 
					permits(AUTHENTICATED_GROUP_NAME, auth.ACT_READ))
    ...
    def test_malformed_acl_file_denies_all(self):
        ...
        acl_prov = IACLProvider(self.library_entry)
        assert_that(acl_prov, 
					denies(AUTHENTICATED_GROUP_NAME, auth.ACT_READ))
    ...
    def test_specific_acl_file(self):
        ...
        for context in self.library_entry, self.library_entry.children[0]:
            ...
            acl_prov = IACLProvider(context)
            assert_that(acl_prov, permits("User", auth.ACT_CREATE))
            assert_that(acl_prov, denies("OtherUser", auth.ACT_CREATE))

        # Now, with an NTIID
        self.library_entry.ntiid = u'tag:nextthought.com,2011-10:PRMIA-HTML-Volume_III.A.2_converted.the_prm_handbook_volume_iii'
        acl_prov = IACLProvider(self.library_entry)
        assert_that(acl_prov, permits("User", auth.ACT_CREATE))
        assert_that(acl_prov, denies("OtherUser", auth.ACT_CREATE))

        assert_that(acl_prov, 
                    permits("content-role:prmia:Volume_III.A.2_converted.the_prm_handbook_volume_iii".lower(), auth.ACT_READ))
        assert_that(acl_prov, 
                    permits(IGroup("content-role:prmia:Volume_III.A.2_converted.the_prm_handbook_volume_iii".lower()), auth.ACT_READ))
...
class Permits(BaseMatcher):
    ...
    def __init__(self, prin, perm, policy=ACLAuthorizationPolicy()):
        ...
        try:
            self.prin = (IPrincipal(prin),)
        except TypeError:
            self.prin = prin
        self.perm = perm if IPermission.providedBy(perm) else Permission(perm)
        self.policy = policy

    def _matches(self, item):
        if not hasattr(item, '__acl__'):
            item = IACLProvider(item, item)
    ...
    def describe_mismatch(self, item, mismatch_description):
        ...
        if acl is None:
            acl = getattr(IACLProvider(item, item), '__acl__', None)
