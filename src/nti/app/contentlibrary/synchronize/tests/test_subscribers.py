...
class TestSubscribers(ApplicationLayerTest):
    ...
    def setUp(self):
        ...
        self.library_dir = os.path.join(os.path.dirname(__file__),
										'library')
    ...
    def test_indexing(self, mock_registry, mock_sites):
        ...
        assert_that((x.ntiid for x in results),
                    contains_inanyorder(
			            'tag:nextthought.com,2011-10:NTI-NTIVideo-CourseTestContent.ntivideo.video1',
			            'tag:nextthought.com,2011-10:OU-JSON:Timeline-CourseTestContent.timeline.heading_west',
			            'tag:nextthought.com,2011-10:OU-NTISlideDeck-CourseTestContent.nsd.pres:Nested_Conditionals',
			            'tag:nextthought.com,2011-10:NTI-RelatedWorkRef-CourseTestContent.relatedworkref.0',
			            'tag:nextthought.com,2011-10:OU-NTISlideVideo-CourseTestContent.nsd.pres:Nested_Conditionals'))
        last_mod_namespace = _get_file_last_mod_namespace(content_package, 
														 'video_index.json')
    ...
    def test_related_content_index(self, mock_sites):
        ...
        self._test_feed('related_content_index.json', INTIRelatedWorkRef, 372,
                        create_relatedworkref_from_external)
